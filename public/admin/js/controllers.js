 ...
 * EditUsersCtrl
 */
adminApp.controller('editUserCtrl', function($scope,$location,Users, $stateParams) {
    $scope.user = {};
    $scope.params = {};
    $scope.params.path = $stateParams.paraml;
    ...
    $scope.editusrID = function() {
        ...
        $scope.newPost.role = this.user.role;
         $scope.newPost._id = this.user._id;
        console.log($scope.newPost);
...
//        return 'https://localhost:8080/dist/img' + imageId;
// };
});
...
/*edit testimonial*/
adminApp.controller('edittestCtrl', function($scope,$location,Testimonial, $stateParams) {
    $scope.testimonial = {};
    $scope.params = {};
    $scope.params.path = $stateParams.paraml;
    Testimonial.sigledata($scope.params).then(function(res) {
        if (res == null) {
            window.location.href = '/404';
        } 
        else {
            // console.log(res);
            $scope.testimonial.name = res.name;
            $scope.testimonial.image = res.image;
            $scope.testimonial.description = res.description;
           
            $scope.testimonial._id = res._id;

        }
    });
    $scope.edittestID = function() {
         console.log(this.testimonial);
        $scope.newPost = {};
        $scope.newPost.name = this.testimonial.name;
        $scope.newPost.image = this.testimonial.image;
        $scope.newPost.description = this.testimonial.description;
        
         $scope.newPost._id = this.testimonial._id;
        console.log($scope.newPost);
        Testimonial.update($scope.newPost).then(function(res) {
...
adminApp.controller('InnovateListCtrl', function($scope,InnovateList,Innovate) {
    
    $scope.posts = InnovateList;
    $scope.activePost = false;
    $scope.setActive = function(user) {
        $scope.activePost = user;
        console.log($scope.activePost);
    ...
    $scope.deletepost = function(id) {
        $scope.data={};
         $scope.data.id=id;
        // console.log($scope.data);
        Innovate.remove($scope.data).then(function(res) {
            console.log(res);
            if (res) {
                alert(res.message);
                window.location.reload();
