...
app.controller('InnovateCtrl', function($scope,Innovate) {
    console.log('InnovateCtrl')
    $scope.post ={}
    
    $scope.addinnovate = function() {
        console.log(this.post)
        $scope.newPost = {};
        $scope.newPost.name = this.post.name;
        $scope.newPost.email = this.post.email;
        ...
        Innovate.add($scope.newPost).then(function(res) {
            console.log(res);
            $scope.success_msg = "Thanks for contacting us, We will be get back to you soon!"
        });
        $scope.post = {};
